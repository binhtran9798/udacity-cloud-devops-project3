...
export POSTGRES_PASSWORD=$(kubectl get secret --namespace default udacity1-postgresql -o jsonpath="{.data.postgres-password}" | base64 -d)
echo $POSTGRES_PASSWORD

# Connecting Via Port Forwarding
kubectl port-forward --namespace default svc/udacity1-postgresql 5432:5432 & 
PGPASSWORD=iQXNnjzyLW psql --host 127.0.0.1 -U postgres -d postgres -p 5432

# Connecting Via a Pod

kubectl exec -it udacity-postgre-postgresql-0 bash 
PGPASSWORD="iQXNnjzyLW" psql postgres://postgres@udacity1-postgresql:5432/postgres -c \l
...
DB_USERNAME=postgres DB_PASSWORD=iQXNnjzyLW python app.py
